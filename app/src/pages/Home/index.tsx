...
type UserProps =
  | ({ repos_url: string; followers_url: string } & HeaderProps & CardProps)
  | null;
...
export function Home() {
  ...
  useEffect(() => {
    function getData(url: string): Promise<TypePromise> {
      const promise: Promise<TypePromise> = new Promise(
    ...
    (getData(url) as Promise<UserProps>)
      .then(async (user) => {
        setUser(user);
        if (user) {
          return Promise.all([
            (getData(user.repos_url) as Promise<RepositoryProps[]>).then(
              (repos) => setRepos(repos)
            ),
            (getData(user.followers_url) as Promise<ListUsersProps[]>).then(
              (followers) => {
                setFollowers(followers);
                setLoading(false); // para de carregar após obter todos os dados
              }
            ),
          ]);
        }
      ...
      .catch((error) => {
        setLoading(false); // para de carregar se houver falha
        console.error(error);
      });
  }, []);
    ...
    >
      ...
      <Footer
        {...({
          ...
          hrefLinkedin:
            "https://www.linkedin.com/in/carlos-eduardo-moura-gomes-bb0ab7250/",
          ...
          hrefLicence:
            "https://github.com/cemgthedev/GitHub-Stalker/blob/main/LICENSE",
        } as FooterProps)}
      />
    </div>
  ) : (
